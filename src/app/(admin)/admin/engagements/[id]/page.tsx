  ...
  return (
    <div>
      ...
      <div className="bg-surface border border-gray-200 rounded-xl p-6 mb-6">
        <h2 className="text-sm font-semibold text-secondary uppercase tracking-wide mb-4">Engagement Workflow</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Link href={`/admin/engagements/${id}/import`} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all text-center">
            <div className="text-2xl mb-2">📊</div>
            <div className="font-semibold text-navy text-sm">Import Payroll</div>
            <div className="text-secondary text-xs mt-1">Upload employee CSV</div>
          </Link>
          <Link href={`/admin/engagements/${id}/import-contractors`} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all text-center">
            <div className="text-2xl mb-2">🏢</div>
            <div className="font-semibold text-navy text-sm">Import Contractors</div>
            <div className="text-secondary text-xs mt-1">Add contractor spend</div>
          </Link>
          <Link href={`/admin/engagements/${id}/import-supplies`} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all text-center">
            <div className="text-2xl mb-2">🔬</div>
            <div className="font-semibold text-navy text-sm">Import Supplies</div>
            <div className="text-secondary text-xs mt-1">Add supply expenses</div>
          </Link>
          <Link href={`/admin/engagements/${id}/projects`} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 hover:shadow-sm transition-all text-center">
            <div className="text-2xl mb-2">🧩</div>
            <div className="font-semibold text-navy text-sm">Business Components</div>
            <div className="text-secondary text-xs mt-1">Assign QREs + Four-Part Test</div>
          </Link>
        ...
        <div className="mt-4 flex items-center gap-2 text-xs text-secondary">
          <span className="bg-gray-200 text-body-text px-2 py-0.5 rounded-full font-medium">Step 1</span>
          <span>Import data</span>
          <span className="text-gray-300">→</span>
          <span className="bg-gray-200 text-body-text px-2 py-0.5 rounded-full font-medium">Step 2</span>
          <span>Assign to business components</span>
          <span className="text-gray-300">→</span>
          <span className="bg-gray-200 text-body-text px-2 py-0.5 rounded-full font-medium">Step 3</span>
          <span>Run four-part test</span>
          <span className="text-gray-300">→</span>
          <span className="bg-gray-200 text-body-text px-2 py-0.5 rounded-full font-medium">Step 4</span>
        ...
        {eng.referralSource && eng.referralSource !== "direct" && (
          <div className="bg-white rounded-xl border border-gray-200 p-6 md:col-span-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p className="text-xs text-secondary">Referral Source</p>
                <p className="font-semibold text-navy text-sm">{eng.referralSource}</p>
              </div>
              {eng.referringCpaName && <div>
                <p className="text-xs text-secondary">Referring CPA</p>
                <p className="font-semibold text-navy text-sm">{eng.referringCpaName}</p>
              </div>}
              {eng.referringCpaFirm && <div>
                <p className="text-xs text-secondary">CPA Firm</p>
                <p className="font-semibold text-navy text-sm">{eng.referringCpaFirm}</p>
              </div>}
              {eng.referringCpaEmail && <div>
                <p className="text-xs text-secondary">CPA Email</p>
                <p className="font-semibold text-navy text-sm">{eng.referringCpaEmail}</p>
              </div>}
              <div>
                <p className="text-xs text-secondary">Referral Fee Paid</p>
                <span className={"text-xs px-2 py-1 rounded-full font-medium "+(eng.referralFeePaid?"bg-green-100 text-green-700":"bg-surface text-primary")}>
                  {eng.referralFeePaid?"Paid":"Pending"}
                </span>
              </div>
              {eng.referralFeeAmount && <div>
                <p className="text-xs text-secondary">Fee Amount</p>
                <p className="font-semibold text-navy text-sm">${Number(eng.referralFeeAmount).toLocaleString()}</p>
              </div>}
            </div>
