...
export default function SupplyImport({ engagementId }: { engagementId: string }) {
  ...
  if (status === "success") return (
    <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center">
      ...
      <h2 className="text-2xl font-black text-navy mb-2">{imported} Supply Items Imported</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-surface border-b border-gray-200">
                <tr>{["Status","Description","Amount","GL Account","Qualified"].map(h=><th key={h} className="px-4 py-2 text-left text-xs font-medium text-secondary uppercase">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {rows.map((row,i)=>(
                  <tr key={i} className={row.valid?"":"bg-red-50"}>
                    ...
                    <td className="px-4 py-2 font-medium text-navy">{row.description}</td>
                    <td className="px-4 py-2 text-navy">${row.amount.toLocaleString()}</td>
                    <td className="px-4 py-2 text-secondary text-xs">{row.glAccount||"--"}</td>
                    <td className="px-4 py-2"><span className={"text-xs px-2 py-0.5 rounded-full font-medium "+(row.qualified?"bg-green-100 text-green-700":"bg-surface text-primary")}>{row.qualified?"Yes":"Pending"}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
