...
export default function ContractorImport({ engagementId }: { engagementId: string }) {
  ...
  if (status === "success") return (
    <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center">
      ...
      <h2 className="text-2xl font-black text-navy mb-2">{imported} Contractor Items Imported</h2>
      <p className="text-secondary mb-8">Records created and ready to assign to business components.</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <h2 className="font-bold text-navy mb-4">Upload CSV File</h2>
        <input type="file" accept=".csv,.txt" onChange={e=>{const f=e.target.files?.[0];if(f){const r=new FileReader();r.onload=ev=>parseCSV(ev.target?.result as string);r.readAsText(f);}}} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm" />
      </div>
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <h2 className="font-bold text-navy mb-2">Or Paste CSV Data</h2>
        <textarea onChange={e=>parseCSV(e.target.value)} placeholder={"vendorName,amount,gl_account,usBasedFlag\nAWS cloud compute,48000,6200,true\n3D printing materials,12000,6300,true"} rows={5} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
      </div>
      {rows.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
            <h2 className="font-bold text-navy">Preview ({rows.length} rows)</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-surface border-b border-gray-200">
                <tr>{["Status","Vendor","Amount","Contract Ref","US Based"].map(h=><th key={h} className="px-4 py-2 text-left text-xs font-medium text-secondary uppercase">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {rows.map((row,i)=>(
                  <tr key={i} className={row.valid?"":"bg-red-50"}>
                    ...
                    <td className="px-4 py-2 font-medium text-navy">{row.vendorName}</td>
                    <td className="px-4 py-2 text-navy">${row.amount.toLocaleString()}</td>
                    <td className="px-4 py-2 text-secondary text-xs">{row.contractReference||"--"}</td>
                    <td className="px-4 py-2"><span className={"text-xs px-2 py-0.5 rounded-full font-medium "+(row.usBasedFlag?"bg-green-100 text-green-700":"bg-surface text-primary")}>{row.usBasedFlag?"Yes":"Pending"}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="px-6 py-4 border-t border-gray-100 flex justify-between items-center">
            <p className="text-secondary text-sm">{rows.filter(r=>r.valid).length} rows will be imported</p>
