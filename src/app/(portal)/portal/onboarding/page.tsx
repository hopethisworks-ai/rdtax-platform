    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-slate-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden">
        ...
        <div className="px-8 py-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">{STEPS[step].title}</h1>
          <p className="text-slate-500 mb-8">{STEPS[step].description}</p>
          ...
          {step === 0 && (
            <div className="space-y-4">
              <div className="flex items-start gap-4 p-4 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <div>
                  <h3 className="font-semibold text-slate-800">What is the R&D Tax Credit?</h3>
                  <p className="text-sm text-slate-600 mt-1">The federal R&D tax credit (IRC §41) rewards companies for investing in qualified research activities. Credits typically range from 6–10% of qualifying R&D spend and can be worth hundreds of thousands of dollars.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4 p-4 bg-amber-50 rounded-xl border border-amber-100">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600 mt-1">All documents are encrypted and stored securely. Your financial data is never shared without your consent.</p>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {DOC_CATEGORIES.map((doc) => (
                <div key={doc.label} className="flex items-start gap-3 p-3 border border-slate-200 rounded-xl hover:border-blue-200 transition-colors">
                  <div className="text-xl flex-shrink-0">{doc.icon}</div>
                  <div>
                    <p className="text-sm font-semibold text-slate-800">{doc.label}</p>
                    <p className="text-xs text-slate-500 mt-0.5 leading-snug">{doc.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {step === 3 && (
            <div className="text-center py-4">
              ...
              <p className="text-slate-600 mb-6">Your portal is set up and your analyst has been notified. Head to your dashboard to view your engagement, upload documents, and track your study progress.</p>
              <div className="bg-slate-50 rounded-xl p-4 text-left space-y-2">
                <p className="text-sm font-semibold text-slate-700">Quick tips:</p>
                <p className="text-sm text-slate-500">• Upload documents in the <strong>Upload</strong> section of your engagement</p>
                <p className="text-sm text-slate-500">• Check the <strong>Documents</strong> checklist to see what's still needed</p>
                <p className="text-sm text-slate-500">• Your analyst will reach out if they have questions</p>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between mt-8">
            <button
              onClick={back}
              className={`px-5 py-2.5 text-sm font-medium rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors ${step === 0 ? "invisible" : ""}`}
            >
              Back
            </button>
            <button
