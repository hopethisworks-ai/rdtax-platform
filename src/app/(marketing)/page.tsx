        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <Link href="/contact" className="bg-white border border-stone-200 rounded-2xl p-8 hover:shadow-lg transition-shadow group">
              ...
              <span className="inline-flex items-center text-teal-600 text-sm font-semibold group-hover:underline">Get assessed <ArrowRight /></span>
            </Link>
            <Link href="/cpa-partners" className="bg-white border border-stone-200 rounded-2xl p-8 hover:shadow-lg transition-shadow group">
              ...
              <span className="inline-flex items-center text-teal-600 text-sm font-semibold group-hover:underline">Partner with us <ArrowRight /></span>
            </Link>
            <Link href="/contact" className="bg-white border border-stone-200 rounded-2xl p-8 hover:shadow-lg transition-shadow group">
              <div className="w-12 h-12 bg-teal-50 rounded-xl flex items-center justify-center mb-5">
                <svg className="w-6 h-6 text-teal-600" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" /></svg>
              </div>
              ...
              <span className="inline-flex items-center text-teal-600 text-sm font-semibold group-hover:underline">Learn more <ArrowRight /></span>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section id="how-it-works" className="py-24 px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-5 gap-6">
            {[
              { num: "01", title: "Discovery", desc: "Free 30-minute call to understand your business and estimate potential credit.", icon: <svg className="w-6 h-6 text-teal-600" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" /></svg> },
              ...
              { num: "03", title: "Interviews & Data", desc: "Technical interviews with your team plus secure data collection through our portal.", icon: <svg className="w-6 h-6 text-teal-600" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 0 1-.825-.242m9.345-8.334a2.126 2.126 0 0 0-.476-.095 48.64 48.64 0 0 0-8.048 0c-1.131.094-1.976 1.057-1.976 2.192v4.286c0 .837.46 1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455 48.455 0 0 0 11.25 3c-2.115 0-4.198.137-6.24.402-1.608.209-2.76 1.614-2.76 3.235v6.226c0 1.621 1.152 3.026 2.76 3.235.577.075 1.157.14 1.74.194V21l4.155-4.155" /></svg> },
              { num: "04", title: "Calculation & Report", desc: "Engineering analysis, QRE calculation, and a complete IRS-compliant credit study.", icon: <svg className="w-6 h-6 text-teal-600" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg> },
              { num: "05", title: "Filing Support", desc: "We coordinate with your CPA to file and provide full audit defense if needed.", icon: <svg className="w-6 h-6 text-teal-600" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" /></svg> },
            ].map((step) => (
              <div key={step.num}>
                <div className="w-11 h-11 bg-teal-50 rounded-xl flex items-center justify-center mb-4">{step.icon}</div>
                <div className="text-xs font-black text-teal-600 uppercase tracking-widest mb-2">Step {step.num}</div>
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={item.title} className="flex gap-3">
                  <div className="flex-shrink-0 mt-1"><CheckIcon className="w-5 h-5 text-teal-600" /></div>
                  <div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={study.type} className="bg-stone-50 border border-stone-200 rounded-2xl p-8">
                <div className="text-3xl font-black text-teal-600 mb-1">{study.amount}</div>
                ...
                <span className="inline-block bg-teal-50 text-teal-700 text-xs font-medium px-3 py-1 rounded-full">{study.period}</span>
              </div>
            ))}
          </div>
            ...
            {[
              ...
              <details key={faq.q} className="bg-white rounded-xl p-5 cursor-pointer hover:shadow-md transition-shadow group border border-slate-100">
                <summary className="flex items-center justify-between font-bold text-slate-900">
                  <span>{faq.q}</span>
                  <svg className="w-5 h-5 text-teal-600 group-open:rotate-180 transition-transform flex-shrink-0 ml-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <section className="py-20 px-6 bg-teal-50 border-t border-teal-100">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact" className="bg-teal-600 text-white font-bold px-10 py-4 rounded-xl text-lg hover:bg-teal-700 transition-colors shadow-md">
              Schedule Consultation
            </Link>
            <Link href="/estimator" className="border-2 border-stone-300 text-slate-700 font-bold px-10 py-4 rounded-xl text-lg hover:border-teal-600 hover:text-teal-600 transition-colors">
              Estimate My Credit
            </Link>
          </div>
        </div>
